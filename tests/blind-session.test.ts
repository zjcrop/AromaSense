...
import { cuppingModeFromMetadata, normalizeSessionMetadata, type CuppingMode } from "../app/core/session-metadata";
...
function metadata(cuppingMode: CuppingMode) {
...
test("blind mode exposes only anonymous sample code until session completion", () => {
  const sessionMetadata = metadata("blind");
...
test("cupping mode survives setup draft normalization", () => {
  ...
  assert.equal(draft?.sessionMetadata.cuppingMode, "blind");
});

test("repository persists canonical cupping mode and reveal timestamp in existing session metadata JSON", async () => {
  ...
  try {
    ...
    const session = createSession({ sessionId: "blind-persist", metadata: metadata("blind"), now });
    ...
    assert.equal(stored.metadata.cuppingMode, "blind");
    ...
    assert.equal(restored.metadata.cuppingMode, "blind");
...
test("creating a new blind session from previously revealed metadata clears stale reveal state", () => {
  ...
  assert.equal(created.metadata.cuppingMode, "blind");
