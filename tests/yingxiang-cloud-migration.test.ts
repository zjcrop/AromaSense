...
function sql(path: string): string { return readFileSync(path, "utf8"); }
function policy(uniqueWithinEvent = true): string {
  return JSON.stringify({
    schemaVersion: "yingxiang-event-policy/0.1", allowGuestParticipants: true,
    participantName: { mode: "participant_choice", allowAccountDisplayName: true, uniqueWithinEvent, minLength: 1, maxLength: 24 },
    revealSampleIdentity: "on_event_complete", calibrationRepeatEnabled: true
  });
}
function manifest(): string {
  return JSON.stringify({ schemaVersion: "yingxiang-event-manifest/0.1", organizerName: "测试组织方", cuppingMode: "blind", samples: [
    { eventSampleId: "slot-001", sampleCode: "101", order: 1 }, { eventSampleId: "slot-002", sampleCode: "205", order: 2 }
  ]});
}
...
test("Yingxiang D1 migrations execute and enforce invite, calibration, host and sequence guards", () => {
  ...
  try {
    ...
    db.prepare(`INSERT INTO yingxiang_participants
      (participant_id, join_request_id, event_id, invite_id, account_user_id, identity_kind, display_name, status, joined_at, participant_ordinal)
      VALUES (?, ?, ?, ?, NULL, 'guest', ?, 'active', ?, ?)`)
      .run("participant-1", "join-request-0001", "event-1", "invite-1", "P01", "2026-09-06T00:02:00.000Z", 1);
    ...
    assert.throws(() => db.prepare(`INSERT INTO yingxiang_participants
      (participant_id, join_request_id, event_id, invite_id, account_user_id, identity_kind, display_name, status, joined_at, participant_ordinal)
      VALUES (?, ?, ?, ?, NULL, 'guest', ?, 'active', ?, ?)`)
      .run("participant-2", "join-request-0002", "event-1", "invite-1", "P02", "2026-09-06T00:03:00.000Z", 1), /UNIQUE/i);
