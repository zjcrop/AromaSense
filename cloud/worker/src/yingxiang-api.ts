...
function parsePolicy(value: unknown): EventPolicy | undefined {
  ...
  const n = naming as Record<string, unknown>; const minLength = Number(n.minLength); const maxLength = Number(n.maxLength);
  ...
  return {
    ...
    participantName: { mode: n.mode, allowAccountDisplayName: n.allowAccountDisplayName, uniqueWithinEvent: n.uniqueWithinEvent, minLength, maxLength, ...(requiredPrefix ? { requiredPrefix } : {}) },
    revealSampleIdentity: source.revealSampleIdentity, calibrationRepeatEnabled: source.calibrationRepeatEnabled
  };
}
...
function parseManifest(value: unknown, requireSamples = true): EventManifest | undefined {
  ...
  const samples: EventSample[] = []; const ids = new Set<string>(); const codes = new Set<string>(); const orders = new Set<number>();
...
function principalPayload(row: ParticipantRow): Record<string, unknown> {
  return {
    ...
    accountDisplayNameHidden: true, status: row.status, boundAt: row.joined_at, releasedAt: row.released_at ?? undefined
  };
}
...
async function inviteFromToken(db: D1Database, token: string): Promise<{ invite: InviteRow; event: EventRow; policy: EventPolicy; manifest: EventManifest } | undefined> {
  ...
  const row = await db.prepare(`SELECT i.invite_id, i.event_id, i.event_revision, i.assigned_name, i.expires_at, i.max_uses, i.use_count, i.revoked_at,
    ...
    FROM yingxiang_invites i JOIN yingxiang_events e ON e.event_id = i.event_id WHERE i.token_hash = ?1`).bind(hash).first<Record<string, unknown>>();
  if (!row) return undefined;
  const event: EventRow = {
    event_id: String(row.event_id), owner_user_id: String(row.owner_user_id), event_revision: Number(row.current_event_revision), title: String(row.title),
    status: String(row.status) as EventStatus, policy_json: String(row.policy_json), manifest_json: String(row.manifest_json), created_at: String(row.created_at), updated_at: String(row.updated_at)
  };
  const contracts = eventContracts(event); if (!contracts) throw new Error("YINGXIANG_EVENT_CONTRACT_CORRUPT");
  return { invite: {
    invite_id: String(row.invite_id), event_id: String(row.event_id), event_revision: Number(row.event_revision), assigned_name: row.assigned_name === null ? null : String(row.assigned_name),
    expires_at: String(row.expires_at), max_uses: row.max_uses === null ? null : Number(row.max_uses), use_count: Number(row.use_count), revoked_at: row.revoked_at === null ? null : String(row.revoked_at)
  }, event, ...contracts };
}
...
async function handleInviteJoin(token: string, request: Request, db: D1Database, user?: YingxiangAuthenticatedUser): Promise<Response> {
  ...
  if (existing) {
    ...
    if (existing.event_id !== resolved.event.event_id || existing.invite_id !== resolved.invite.invite_id || !sameAccount) {
...
async function handleEventCreate(request: Request, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  const body = await parseJsonObject(request); if (!body) return json({ ok: false, error: "INVALID_JSON" }, 400);
  const title = normalizeName(body.title); const policy = parsePolicy(body.policy); const status: EventStatus = body.publish === false ? "draft" : "published";
...
async function handleInviteCreate(eventId: string, request: Request, db: D1Database, user: YingxiangAuthenticatedUser, publicAppUrl?: string): Promise<Response> {
  ...
  const expiresAt = new Date(expiryMs).toISOString(); const inviteId = crypto.randomUUID(); const token = randomHex(24); const tokenHash = await sha256Hex(token); const now = new Date().toISOString();
  await db.prepare(`INSERT INTO yingxiang_invites
    ...
    VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, 0, ?8, NULL)`).bind(inviteId, eventId, event.event_revision, tokenHash, assignedName, expiresAt, maxUsesValue, now).run();
  ...
  return json({ ok: true, inviteId, eventId, eventRevision: event.event_revision, token, expiresAt, maxUses: maxUsesValue, share: {
    deepLink: `aromasense://yingxiang/invite/${token}`, webUrl: appBase ? `${appBase}/?yingxiangInvite=${encodeURIComponent(token)}` : undefined
  }}, 201);
}
...
async function handleAccountDisplayName(request: Request, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  ...
  await db.prepare("UPDATE users SET display_name = ?1 WHERE user_id = ?2").bind(displayName, user.userId).run(); return json({ ok: true, displayName });
}
...
async function handleCalibrationCreate(eventId: string, request: Request, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  ...
  try {
    await db.prepare(`INSERT INTO yingxiang_calibration_groups (group_id, event_id, canonical_sample_id, event_sample_ids_json, reveal_policy, created_at)
      VALUES (?1, ?2, ?3, ?4, ?5, ?6)`).bind(groupId, eventId, canonicalSampleId, JSON.stringify(eventSampleIds), revealPolicy, now).run();
  } catch (error) {
    const message = error instanceof Error ? error.message : "";
...
async function handleEventComplete(eventId: string, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  ...
  const updated = await eventById(db, eventId); return json({ ok: true, eventId, status: "completed", eventRevision: updated?.event_revision, releasedAt: now });
}
...
export async function handleYingxiangAuthenticatedRoute(request: Request, url: URL, db: D1Database, user: YingxiangAuthenticatedUser, publicAppUrl?: string): Promise<Response | undefined> {
  ...
  const invite = url.pathname.match(/^\/api\/v1\/yingxiang\/events\/([^/]+)\/invites$/u); if (invite && request.method === "POST") return handleInviteCreate(decodeURIComponent(invite[1]), request, db, user, publicAppUrl);
