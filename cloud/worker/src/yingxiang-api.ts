...
type CuppingMode = "open" | "blind" | "semi_blind";
...
function parsePolicy(value: unknown): EventPolicy | undefined {
  ...
  const minLength = Number(n.minLength); const maxLength = Number(n.maxLength);
  ...
  return {
    schemaVersion: "yingxiang-event-policy/0.1", allowGuestParticipants: true,
    participantName: {
      mode: n.mode, allowAccountDisplayName: n.allowAccountDisplayName, uniqueWithinEvent: n.uniqueWithinEvent,
      minLength, maxLength, ...(requiredPrefix ? { requiredPrefix } : {})
    },
...
function parseManifest(value: unknown, requireSamples = true): EventManifest | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const source = value as Record<string, unknown>;
  const organizerName = normalizeName(source.organizerName);
  const cuppingMode = source.cuppingMode;
  const rawSamples = source.samples;
  if (source.schemaVersion !== "yingxiang-event-manifest/0.1" || !organizerName || Array.from(organizerName).length > 120
    || (cuppingMode !== "open" && cuppingMode !== "blind" && cuppingMode !== "semi_blind") || !Array.isArray(rawSamples)
    || rawSamples.length > 300 || (requireSamples && rawSamples.length === 0)) return undefined;
  const samples: EventSample[] = [];
  const ids = new Set<string>(); const codes = new Set<string>(); const orders = new Set<number>();
  for (const item of rawSamples) {
    if (!item || typeof item !== "object" || Array.isArray(item)) return undefined;
    const row = item as Record<string, unknown>;
    const eventSampleId = normalizeName(row.eventSampleId); const sampleCode = normalizeName(row.sampleCode);
    const order = Number(row.order); const label = row.label === undefined ? undefined : normalizeName(row.label);
    if (!eventSampleId || eventSampleId.length > 128 || !sampleCode || sampleCode.length > 128
      || !Number.isSafeInteger(order) || order < 1 || (row.label !== undefined && !label)
      || ids.has(eventSampleId) || codes.has(sampleCode) || orders.has(order)) return undefined;
  ...
  return { schemaVersion: "yingxiang-event-manifest/0.1", organizerName, cuppingMode, samples };
}

function validateDisplayName(value: unknown, policy: ParticipantNamePolicy): string | undefined {
  const name = normalizeName(value); const length = Array.from(name).length;
...
async function eventById(db: D1Database, eventId: string): Promise<EventRow | null> {
  return db.prepare(`SELECT event_id, owner_user_id, event_revision, title, status, policy_json, manifest_json, created_at, updated_at
...
function eventContracts(row: EventRow): { policy: EventPolicy; manifest: EventManifest } | undefined {
  const policy = parsePolicy(parseStoredJson(row.policy_json));
  const manifest = parseManifest(parseStoredJson(row.manifest_json), row.status !== "draft");
...
function publicEvent(row: EventRow, policy: EventPolicy, manifest: EventManifest): Record<string, unknown> {
  return {
    ...
    title: row.title, status: row.status, policy, manifest, createdAt: row.created_at, updatedAt: row.updated_at
  };
}

async function inviteFromToken(db: D1Database, token: string): Promise<{ invite: InviteRow; event: EventRow; policy: EventPolicy; manifest: EventManifest } | undefined> {
  ...
  const row = await db.prepare(`SELECT
      ...
      e.owner_user_id, e.event_revision AS current_event_revision, e.title, e.status, e.policy_json, e.manifest_json, e.created_at, e.updated_at
    FROM yingxiang_invites i JOIN yingxiang_events e ON e.event_id = i.event_id WHERE i.token_hash = ?1`)
  ...
  const event: EventRow = {
    ...
    title: String(row.title), status: String(row.status) as EventStatus, policy_json: String(row.policy_json),
    manifest_json: String(row.manifest_json), created_at: String(row.created_at), updated_at: String(row.updated_at)
  };
  const contracts = eventContracts(event); if (!contracts) throw new Error("YINGXIANG_EVENT_CONTRACT_CORRUPT");
  return {
    invite: {
...
async function handleInviteJoin(token: string, request: Request, db: D1Database, user?: YingxiangAuthenticatedUser): Promise<Response> {
  ...
  if (naming.mode === "organizer_assigned") {
    ...
    const verified = await accountDisplayName(db, user.userId);
    if (!verified) return json({ ok: false, error: "YINGXIANG_ACCOUNT_NAME_UNAVAILABLE" }, 409);
    displayName = validateDisplayName(verified, naming);
  ...
  return json({ ok: true, principal: {
    schemaVersion: "yingxiang-principal/0.1", participantId, eventId: resolved.event.event_id, identityKind,
    accountUserId: user?.userId, displayName, accountDisplayNameHidden: true, status: "active", boundAt: now
  }, event: publicEvent(resolved.event, resolved.policy, resolved.manifest) }, 201);
}

async function handleEventCreate(request: Request, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  const body = await parseJsonObject(request); if (!body) return json({ ok: false, error: "INVALID_JSON" }, 400);
  const title = normalizeName(body.title); const policy = parsePolicy(body.policy);
  const status: EventStatus = body.publish === false ? "draft" : "published";
  const manifest = parseManifest(body.manifest, status !== "draft");
  if (!title || Array.from(title).length > 120 || !policy || !manifest) return json({ ok: false, error: "YINGXIANG_EVENT_PAYLOAD_INVALID" }, 400);
  ...
  await db.prepare(`INSERT INTO yingxiang_events
    ...
    .bind(eventId, user.userId, title, status, JSON.stringify(policy), JSON.stringify(manifest), now).run();
  const row = await eventById(db, eventId);
  return json({ ok: true, event: publicEvent(row!, policy, manifest) }, 201);
}
...
async function handleInviteCreate(eventId: string, request: Request, db: D1Database, user: YingxiangAuthenticatedUser, publicAppUrl?: string): Promise<Response> {
  ...
  if (contracts.policy.participantName.mode === "organizer_assigned" && !validateDisplayName(assignedName, contracts.policy.participantName)) {
  ...
  const expiresAt = new Date(expiryMs).toISOString(); const inviteId = crypto.randomUUID();
  const token = randomHex(24); const tokenHash = await sha256Hex(token); const now = new Date().toISOString();
  ...
  return json({ ok: true, inviteId, eventId, eventRevision: event.event_revision, token, expiresAt, maxUses: maxUsesValue, share: {
...
async function handleAccountDisplayName(request: Request, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  const body = await parseJsonObject(request); if (!body) return json({ ok: false, error: "INVALID_JSON" }, 400);
  const displayName = normalizeName(body.displayName); const length = Array.from(displayName).length;
...
async function handleCalibrationCreate(eventId: string, request: Request, db: D1Database, user: YingxiangAuthenticatedUser): Promise<Response> {
  ...
  if (!contracts) return json({ ok: false, error: "YINGXIANG_EVENT_CONTRACT_CORRUPT" }, 500);
  ...
  if (eventSampleIds.length < 2 || eventSampleIds.some((id) => !id || !allowed.has(id)) || new Set(eventSampleIds).size !== eventSampleIds.length) {
  ...
  const groupId = crypto.randomUUID(); const now = new Date().toISOString();
  try {
    ...
    const message = error instanceof Error ? error.message : "";
    if (message.includes("YINGXIANG_CALIBRATION_")) return json({ ok: false, error: message.match(/YINGXIANG_CALIBRATION_[A-Z_]+/u)?.[0] ?? "YINGXIANG_CALIBRATION_INVALID" }, 400);
    throw error;
  }
...
export async function handleYingxiangAuthenticatedRoute(
  request: Request, url: URL, db: D1Database, user: YingxiangAuthenticatedUser, publicAppUrl?: string
  ...
  const invite = url.pathname.match(/^\/api\/v1\/yingxiang\/events\/([^/]+)\/invites$/u);
  if (invite && request.method === "POST") return handleInviteCreate(decodeURIComponent(invite[1]), request, db, user, publicAppUrl);
  const calibration = url.pathname.match(/^\/api\/v1\/yingxiang\/events\/([^/]+)\/calibration-groups$/u);
  if (calibration && request.method === "POST") return handleCalibrationCreate(decodeURIComponent(calibration[1]), request, db, user);
  const complete = url.pathname.match(/^\/api\/v1\/yingxiang\/events\/([^/]+)\/complete$/u);
  if (complete && request.method === "POST") return handleEventComplete(decodeURIComponent(complete[1]), db, user);
  return undefined;
}
