...
function installStyles(): void {
  ...
  style.textContent = `
    ...
    .yingxiang-join__samples{display:flex;flex-wrap:wrap;gap:7px}.yingxiang-join__sample{display:grid;place-items:center;width:42px;height:42px;box-sizing:border-box;padding:4px;border:1px solid rgba(185,153,90,.22);border-radius:7px;background:#121212;color:#c8c0b4;font:11px/1 ui-monospace,SFMono-Regular,Consolas,monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    ...
    .yingxiang-join__status{min-height:21px;margin:0;color:#b7b0a6;font-size:12px;line-height:1.6;white-space:pre-wrap}.yingxiang-join__notice{margin:0;color:#858078;font-size:11px;line-height:1.55}
...
export class YingxiangJoinRenderer {
  ...
  async render(): Promise<void> {
    ...
    const shell = document.createElement("section"); shell.className = "yingxiang-join";
  ...
  private renderReady(shell: HTMLElement): void {
    ...
    const titleWrap = document.createElement("div"); titleWrap.append(Object.assign(document.createElement("h2"), { className: "yingxiang-join__title", textContent: "迎香 · 加入杯测" }));
    const close = Object.assign(document.createElement("button"), { type: "button", className: "yingxiang-join__close", textContent: "取消" }); close.addEventListener("click", () => this.options.onClose()); head.append(titleWrap, close);

    const event = document.createElement("section"); event.className = "yingxiang-join__event"; event.append(Object.assign(document.createElement("h3"), { textContent: preview.event.title }));
    event.append(Object.assign(document.createElement("div"), { className: "yingxiang-join__meta", textContent: `${preview.event.manifest.organizerName} · ${cuppingModeLabel(preview.event.manifest.cuppingMode)} · ${preview.event.manifest.samples.length} 个样品 · revision ${preview.event.eventRevision}` }));
    const samples = document.createElement("div"); samples.className = "yingxiang-join__samples";
    for (const sample of [...preview.event.manifest.samples].sort((a, b) => a.order - b.order)) samples.append(Object.assign(document.createElement("span"), { className: "yingxiang-join__sample", textContent: sample.sampleCode, title: sample.sampleCode }));
    ...
    let customName: HTMLInputElement | undefined; let accountName: HTMLInputElement | undefined; let source: "custom" | "account" = "custom";

    if (naming.mode === "organizer_assigned") {
      const prefix = preview.invite.namePrefix || naming.requiredPrefix || "参与者";
      const text = preview.invite.assignedName
        ? `本次参与名称：${preview.invite.assignedName}。活动期间香迹只显示该临时身份。`
        : `加入成功后按加入顺序自动获得临时名称，例如 ${prefix}01、${prefix}02。活动期间香迹只显示该临时身份。`;
      form.append(Object.assign(document.createElement("p"), { className: "yingxiang-join__notice", textContent: text }));
    } else {
      ...
      const accountChoice = document.createElement("label"); accountChoice.className = "yingxiang-join__choice"; accountChoice.append(accountRadio, document.createTextNode("使用个人香迹账户名称"));
      ...
      accountName = document.createElement("input"); accountName.type = "text"; accountName.maxLength = 64; accountName.placeholder = "输入并保存为你的香迹账户显示名称";
      const accountField = document.createElement("label"); accountField.className = "yingxiang-join__field"; accountField.hidden = true; accountField.append(document.createTextNode("个人香迹账户显示名称"), accountName); form.append(accountField);
    ...
    form.append(Object.assign(document.createElement("p"), { className: "yingxiang-join__notice", textContent: "加入后会在本机建立一份香迹杯测 Session。迎香活动身份只在本次杯测有效；断网后已建立的本地记录仍可继续。" }));
    const join = Object.assign(document.createElement("button"), { type: "button", className: "yingxiang-join__action", textContent: "加入并开始杯测" });
    join.addEventListener("click", () => void this.join({ join, status, joinRequestId, source, customName, accountName })); form.append(join, status); shell.append(head, event, form);
  }

  private async join(view: { join: HTMLButtonElement; status: HTMLElement; joinRequestId: string; source: "custom" | "account"; customName?: HTMLInputElement; accountName?: HTMLInputElement }): Promise<void> {
    const preview = this.preview!; view.join.disabled = true; view.status.textContent = "正在建立活动身份和本地杯测…";
    try {
      let displayName: string | undefined; let nameSource: "custom" | "account" | undefined;
      if (preview.event.policy.participantName.mode === "participant_choice") {
        ...
        if (nameSource === "account") {
          ...
          if (!value) throw new Error("请输入个人香迹账户显示名称。未登录香迹账户不能使用此选项。");
          await this.client.setAccountDisplayName(value);
        } else {
          displayName = view.customName?.value.normalize("NFKC").trim(); if (!displayName) throw new Error("请输入本次活动名称。");
        }
      }
