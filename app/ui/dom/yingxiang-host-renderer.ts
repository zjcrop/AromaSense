...
import { YingxiangSampleListSorter } from "./yingxiang-sample-list-sorter";
...
export class YingxiangHostRenderer {
  ...
  private sampleSorter?: YingxiangSampleListSorter;
  ...
  render(): void {
    installStyles();
    this.sampleSorter?.dispose();
    this.root.replaceChildren();
    this.event = this.options.initialEvent;
    ...
    copy.append(
      Object.assign(document.createElement("h2"), { className: "yingxiang-host__title", textContent: "迎香" }),
      Object.assign(document.createElement("p"), { className: "yingxiang-host__sub", textContent: "发布多人杯测 · 主办方账号独立 · 参与身份仅在本次活动有效" })
    );
    ...
    eventSection.append(Object.assign(document.createElement("h3"), { textContent: "活动与样品" }));
    ...
    const cuppingMode = document.createElement("select");
    cuppingMode.append(new Option("盲测", "blind"), new Option("半盲测", "semi_blind"), new Option("公开杯测", "open"));
    ...
    const sampleCodeTools = document.createElement("div");
    const sampleSortRoot = document.createElement("div");
    sampleCodeField.append(sampleCodeTools, sampleSortRoot);
    grid.append(
      field("活动名称", eventTitle, true),
      field("组织方显示名称", organizerName),
      field("杯测模式", cuppingMode),
      sampleCodeField
    );
    eventSection.append(
      grid,
      Object.assign(document.createElement("p"), {
        className: "yingxiang-host__hint",
        textContent: "样品编号是活动唯一豆单，可直接输入，也可拍照、上传图片、导入表格或粘贴。下方拖动排序直接改变杯测顺序，不再维护第二份真实豆单或位置映射。"
      })
    );

    const policySection = document.createElement("section");
    policySection.className = "yingxiang-host__section";
    policySection.append(Object.assign(document.createElement("h3"), { textContent: "参与身份规则" }));
    const policyGrid = document.createElement("div");
    policyGrid.className = "yingxiang-host__grid";
    const namingMode = document.createElement("select");
    namingMode.append(new Option("参与者自定义名称", "participant_choice"), new Option("名称前缀 + 加入顺序自动编号", "organizer_assigned"));
    ...
    const allowAccount = checkbox("允许主动使用个人香迹账户名称", true);
    ...
    policySection.append(policyGrid, checks);
    ...
    const inviteButton = Object.assign(document.createElement("button"), { type: "button", className: "yingxiang-host__action", textContent: "生成邀请" });
    inviteSection.append(
      inviteGrid,
      Object.assign(document.createElement("p"), { className: "yingxiang-host__hint", textContent: "选择自动编号时，不再为每个邀请手工指定名称；参与者按成功加入顺序依次获得名称。" }),
      inviteButton
    );
    ...
    if (initial) {
      eventTitle.value = initial.title;
      organizerName.value = initial.manifest.organizerName;
      cuppingMode.value = initial.manifest.cuppingMode;
      sampleCodes.value = initial.manifest.samples.map((sample) => sample.sampleCode).join("\n");
      namingMode.value = initial.policy.participantName.mode;
      prefix.value = initial.policy.participantName.requiredPrefix ?? "";
      maxNameLength.value = String(initial.policy.participantName.maxLength);
      allowAccount.input.checked = initial.policy.participantName.allowAccountDisplayName;
      uniqueName.input.checked = initial.policy.participantName.uniqueWithinEvent;
      calibration.input.checked = initial.policy.calibrationRepeatEnabled;
    ...
    this.sampleSorter = new YingxiangSampleListSorter(sampleSortRoot, sampleCodes);
    this.sampleSorter.render();
    ...
    if (this.options.structureLocked) {
      ...
      this.sampleSorter.setDisabled(true);
      status.textContent = "已有参与者，样品顺序和身份规则已锁定；当前仍可修改活动名称。重新发布后需生成新邀请。";
    } else if (initial) {
      status.textContent = "重新发布后旧邀请失效，请生成新邀请。";
    }

    createButton.addEventListener("click", () => void this.createEvent({
      eventTitle, organizerName, cuppingMode, sampleCodes, namingMode, prefix, maxNameLength,
      allowAccount: allowAccount.input, uniqueName: uniqueName.input, calibration: calibration.input,
      createButton, status, inviteSection
    }));
    ...
    shell.append(head, eventSection, policySection, createButton, status, inviteSection, result);
    this.root.append(shell);
  }

  private async createEvent(view: {
    eventTitle: HTMLInputElement;
    organizerName: HTMLInputElement;
    cuppingMode: HTMLSelectElement;
    sampleCodes: HTMLTextAreaElement;
    namingMode: HTMLSelectElement;
    prefix: HTMLInputElement;
    maxNameLength: HTMLInputElement;
    allowAccount: HTMLInputElement;
    uniqueName: HTMLInputElement;
    ...
    if (!this.client) {
      view.status.textContent = "迎香云端服务尚未配置，发布功能不可用；本地香迹杯测不受影响。";
      return;
    }
    ...
    if (!Number.isSafeInteger(maxLength) || maxLength < 1 || maxLength > 64) {
      view.status.textContent = "参与名称最大长度必须为 1–64。";
      return;
    }
    const automatic = view.namingMode.value === "organizer_assigned";
    const prefix = view.prefix.value.normalize("NFKC").trim() || (automatic ? "参与者" : "");
    if (automatic && Array.from(prefix).length + 2 > maxLength) {
      view.status.textContent = "自动名称前缀过长：至少需要为两位顺序号预留 2 个字符。";
      return;
    }
    ...
    try {
      ...
      view.status.textContent = error instanceof Error ? error.message : "样品列表无效。";
      return;
    }
    ...
    if (this.options.structureLocked && this.event) {
      manifest = this.event.manifest;
      policy = this.event.policy;
    } else if (this.event) {
      // Preserve opaque legacy coffee metadata for old events without exposing or
      // editing it in the new host workflow. New events never create this mapping.
      manifest = {
        ...manifest,
        samples: manifest.samples.map((sample) => {
          const old = this.event!.manifest.samples.find((value) => value.sampleCode === sample.sampleCode);
          return old ? { ...sample, eventSampleId: old.eventSampleId, ...(old.coffee ? { coffee: old.coffee } : {}) } : sample;
        })
      };
    }
    ...
    try {
      ...
      if (error instanceof YingxiangClientError && error.code === "UNAUTHORIZED") {
        view.status.textContent = "迎香主办方会话已失效，请重新输入迎香账号。";
  ...
  private async createInvite(view: {
    maxUses: HTMLInputElement;
    expiryHours: HTMLInputElement;
    ...
    if (!Number.isSafeInteger(uses) || uses < 1 || uses > 10000 || !Number.isFinite(hours) || hours < 1 || hours > 168) {
      view.status.textContent = "参与人数需为 1–10000，有效期需为 1–168 小时。";
      return;
    }
    ...
    try {
      ...
      view.inviteButton.disabled = false;
    }
  }
}
