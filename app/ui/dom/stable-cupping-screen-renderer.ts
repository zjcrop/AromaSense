...
export class CuppingScreenRenderer {
  ...
  private openRecognitionReview(page: RecognizedPage, preview: string | undefined, status: HTMLElement, add: HTMLButtonElement): void {
    ...
    const openAt = (index: number): void => {
      ...
      this.runtimeReview = openBatchReviewDialog({
        ...
        onExit: () => {
          this.runtimeReview?.close(); this.runtimeReview = undefined; add.disabled = false;
          this.setBusyProgress(false, 0, "");
          status.textContent = addedIds.length ? `已加入 ${addedIds.length} 个豆子；其余候选未确认，因此没有写入。` : "已取消本次新增；未确认豆子没有写入。";
        },
        onConfirm: async (value: BatchReviewValue) => {
          const percent = 82 + Math.round(((index + 1) / samples.length) * 15);
          this.setBusyProgress(true, percent, `确认并保存 ${index + 1}/${samples.length}`);
          const metadata: Record<string, unknown> = { ...sample.metadata };
          for (const [key] of EDITABLE_SAMPLE_FIELDS) delete metadata[key];
          for (const [key, fieldValue] of Object.entries(value.fields)) if (fieldValue.trim()) metadata[key] = fieldValue.trim();
          const recognition = metadata.recognition && typeof metadata.recognition === "object" ? metadata.recognition as Record<string, unknown> : {};
          metadata.recognition = { ...recognition, runtimeRosterAddition: true, runtimeUserConfirmed: true, runtimeAddedAt: this.options.now() };
          const id = crypto.randomUUID();
          await this.controller.addSample(id, { label: value.label.trim(), metadata }, this.options.now());
          addedIds.push(id);
          this.runtimeReview?.close(); this.runtimeReview = undefined;
          if (index + 1 < samples.length) { openAt(index + 1); return; }
          this.setBusyProgress(true, 100, "新增豆子确认完成");
