export const BATCH_SETUP_DRAFT_VERSION = 1 as const;
...
export function normalizeBatchSetupDraft(value: unknown): BatchSetupDraft | undefined {
  ...
  const items = source.items.flatMap((item): BatchSetupDraftItem[] => {
    ...
    return [{
      ...
      metadata,
      previewDataUrl: typeof row.previewDataUrl === "string" && row.previewDataUrl.startsWith("data:image/")
        ? row.previewDataUrl
        : undefined,
...
export function firstPendingItemIndex(items: readonly BatchSetupDraftItem[], startAfter = -1): number {
  ...
  for (let index = normalizedStart + 1; index < items.length; index += 1) {
    if (!items[index].confirmed) return index;
  }
