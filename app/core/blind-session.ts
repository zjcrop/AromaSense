...
export function blindModeLabel(mode: BlindMode): string {
  if (mode === "semi_blind") return "半盲";
  if (mode === "full_blind") return "全盲";
  return "公开";
}

export function blindModeDescription(mode: BlindMode): string {
  if (mode === "semi_blind") return "杯测中隐藏样品名称和直接身份信息，仅显示国家、产区、处理法与烘焙度；整场完成后统一揭盲。";
  if (mode === "full_blind") return "杯测中仅显示匿名样品编号，不显示名称或样品元数据；整场完成后统一揭盲。";
  return "杯测过程中显示样品名称与全部已录入信息。";
}

export function isBlindSessionRevealed(
  metadata: Pick<CuppingSessionMetadata, "blindMode" | "revealedAt">,
  status: SessionVisibilityStatus
): boolean {
  const mode = normalizeBlindMode(metadata.blindMode);
...
export function revealBlindSessionMetadata(metadata: CuppingSessionMetadata, now: string): CuppingSessionMetadata {
  ...
  if (normalizeBlindMode(normalized.blindMode) === "open" || normalized.revealedAt) return normalized;
