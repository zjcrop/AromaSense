...
export class SyncQueueStore {
    ...
    await this.db.transaction(async () => {
      await this.db.run(
        `INSERT OR IGNORE INTO revisions (
          revision_id, session_id, sample_id, stage_id, revision_kind, sequence,
          protocol_version, content_hash, payload_json, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [revision.revisionId, revision.sessionId, revision.sampleId ?? null,
          revision.stageId ?? null, revision.revisionKind, revision.sequence,
          revision.protocolVersion, revision.contentHash, JSON.stringify(revision.payload),
          revision.createdAt]
      );

      const existing = await this.db.get<{ content_hash: string }>(
        `SELECT content_hash FROM revisions WHERE revision_id = ?`, [revision.revisionId]
      );
      if (!existing || existing.content_hash !== revision.contentHash) {
        throw new Error(`LOCAL_REVISION_CONFLICT:${revision.revisionId}`);
      }
  ...
  async claimReady(now: string): Promise<PendingRevision | undefined> {
    return this.db.transaction(async () => {
      const row = await this.db.get<QueueRow>(
        `SELECT q.queue_id, q.status, q.attempt_count, q.next_attempt_at, q.last_error,
      ...
      await this.db.run(
        `UPDATE sync_queue SET status='uploading', attempt_count=attempt_count+1,
         updated_at=? WHERE queue_id=?`, [now, row.queue_id]
      );
  ...
  async markSynced(queueId: string, now: string): Promise<void> {
    await this.db.run(
      `UPDATE sync_queue SET status='synced', next_attempt_at=NULL, last_error=NULL, updated_at=? WHERE queue_id=?`,
      [now, queueId]
    );
  }
  ...
  async markConflict(queueId: string, message: string, now: string): Promise<void> {
    await this.db.run(
      `UPDATE sync_queue SET status='conflict', next_attempt_at=NULL, last_error=?, updated_at=? WHERE queue_id=?`,
      [message, now, queueId]
    );
  }
  ...
  async markFailed(queueId: string, error: string, nextAttemptAt: string, now: string): Promise<void> {
    await this.db.run(
      `UPDATE sync_queue SET status='failed', next_attempt_at=?, last_error=?, updated_at=? WHERE queue_id=?`,
      [nextAttemptAt, error, now, queueId]
    );
  }

  async recoverInterrupted(now: string): Promise<void> {
    await this.db.run(
      `UPDATE sync_queue SET status='failed', last_error='INTERRUPTED_UPLOAD',
       next_attempt_at=?, updated_at=? WHERE status='uploading'`, [now, now]
    );
  }

  async counts(): Promise<Record<SyncQueueStatus, number>> {
    const rows = await this.db.all<{ status: SyncQueueStatus; count: number }>(
