...
export interface YingxiangSessionBinding {
  eventId: string;
...
interface SessionBindingRow {
  event_id: string; participant_id: string; session_id: string; bound_at: string;
}
...
function sameEvent(a: YingxiangEvent, b: YingxiangEvent): boolean {
  return a.eventId === b.eventId
    ...
    && manifestSignature(a.manifest) === manifestSignature(b.manifest);
}
...
export class YingxiangEventStore {
  ...
  async bindSession(binding: YingxiangSessionBinding): Promise<"created" | "already_present"> {
    const eventId = binding.eventId.normalize("NFKC").trim();
    const participantId = binding.participantId.normalize("NFKC").trim();
    const sessionId = binding.sessionId.normalize("NFKC").trim();
    const boundAt = binding.boundAt.trim();
    if (!eventId || !participantId || !sessionId || !boundAt) throw new Error("YINGXIANG_SESSION_BINDING_INVALID");
    const existing = await this.getSessionBinding(eventId, participantId);
    if (existing) {
      if (existing.sessionId !== sessionId) throw new Error("YINGXIANG_SESSION_BINDING_CONFLICT");
      return "already_present";
    }
    const principal = await this.getActivePrincipal(eventId, participantId);
    if (!principal) throw new Error("YINGXIANG_ACTIVE_PRINCIPAL_REQUIRED");
    await this.db.run(
      `INSERT INTO yingxiang_session_bindings (event_id, participant_id, session_id, bound_at)
       VALUES (?, ?, ?, ?)`,
      [eventId, participantId, sessionId, boundAt]
    );
    return "created";
  }

  async getSessionBinding(eventId: string, participantId: string): Promise<YingxiangSessionBinding | undefined> {
    const row = await this.db.get<SessionBindingRow>(
      `SELECT event_id, participant_id, session_id, bound_at
       FROM yingxiang_session_bindings WHERE event_id = ? AND participant_id = ?`,
      [eventId, participantId]
    );
    return row ? {
      eventId: row.event_id,
