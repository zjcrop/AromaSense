...
- The current implementation stores a random 256-bit device credential locally after first account creation. The user still types the Yingxiang account name on every entry; the device credential is used only to prevent account-name-only impersonation.
...
- Cross-device recovery of the device-bound host credential is not implemented in B0.2. Do not silently weaken this to account-name-only authentication.

Cloud migration `0010_yingxiang_host_accounts_and_sequence.sql` adds the host-account/token tables. A shadow `users` row preserves the existing `yingxiang_events.owner_user_id` foreign-key contract; it is an implementation identity only and is not an AromaSense personal account.

## Participant naming

`participantName.mode = organizer_assigned` now means **automatic sequence naming**, not one fixed name per invitation.
...
The review list maps imported coffee rows to the previously entered sample codes by current order. It reuses the shared `attachDragReorder` implementation, including the virtual source placeholder, FLIP placeholder movement and final drop placement.
...
- Applied D1 migrations are never edited; B0.2 uses numbered migration `0010`.
- Event structure remains locked once participants exist, including real-coffee mapping, to prevent a sample identity from changing under already collected observations.
